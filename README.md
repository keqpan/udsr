...
Notebook for exporting depth, image, poses and intrinsics from Scannet's .sens data and following rendering is lacated at 
```/scannet_rendering/render_scannet.ipynb```

Notebook for filterind Scannet and creating clear crops is lockated at
...
You shouldn't have any special structure for your data, but in train and test runing commands you have to add ``` --path_A --path_B --path_A_test --path_B_test --A_add_paths (for train A images) --B_add_paths (for train B images) --A_add_paths_test (for test A images) --B_add_paths_test (for test B images)``` , or you can set this passes as default passes in ```options/train_options.py``` it can be more convenient. 
...
You can add ```--use_wandb``` for using - good and easy-to-use logging tool. Before using it you have to create an account on https://wandb.ai/ and log in on you machine.
...
```sh
python main.py --gpu_ids 0,1 --display_freq 20 --print_freq 20 --n_epochs 20 --n_epochs_decay 60 --custom_pathes  --use_scannet --lr 0.0002 --model translation_block --save_all --batch_size 3 --name test --netD n_layers --crop_size_h 256 --crop_size_w 256 --do_train --dataroot path_to_your_dataset --max_distance 5100 --init_type xavier --model_type T
```
...
Before start training main part you have to end train Image Guidance Network and Translation network. Then create a folder with location `checkpoints/your_folder_name` with the followed structure:
    ...
    │   ├── latest_net_G_A_d.pth          # generator   HQ to LQ from translation
...
```
After that you have to add flag `--no_aug` to turn off augmentation and fine-tune network on full-size image and continue training by followed command:
